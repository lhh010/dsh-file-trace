...
* @param ctx - host context carrying the webServer service.
*/
function registerAssetEndpoint(ctx) {
	try {
		const dispose = ctx.webServer.register({
			kind: "exact",
			path: ASSET_PATH,
			handler: async (req, res) => {
				...
				try {
					...
					if (raw === "") {
						send(400, "missing path");
						return;
					}
					let abs;
					if (isAbsolute(raw)) abs = resolve(raw);
					else {
						const sessionParam = url.searchParams.get("session") ?? "";
						if (sessionParam === "") {
							send(400, "relative path requires session");
							return;
						}
						const cwd = sessionWorkspace(ctx, sessionParam);
						if (cwd === void 0) {
							send(404, "session not found");
							return;
						}
						abs = resolve(cwd, raw);
					}
					...
					if (type === void 0) {
						send(404, "unsupported image type");
						return;
					}
	...
	try {
		const dispose = ctx.webServer.register({
			...
			handler: async (req, res) => {
				...
				const rel = decodeURIComponent(url.pathname.slice(25)).replace(/^\/+/, "");
				if (rel === "" || !/.m?js$/.test(rel)) {
					deny(404, "not a js resource");
					return;
				}
				const abs = normalize(join(LIB_DIR, rel));
				const inside = (p) => {
					const r = relative(LIB_DIR, p);
					return r !== "" && !r.startsWith("..") && !isAbsolute(r);
				};
				if (!inside(abs)) {
					deny(403, "path escapes the plugin lib");
					return;
				}
				let file;
				try {
