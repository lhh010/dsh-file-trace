window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function presentOpsOf(args, base) {
			...
			for (const entry of files) {
				...
				const p = entry.path;
				if (typeof p === "string" && p.length > 0) out.push({
					...base,
					path: p,
					presentOnly: true
				});
		...
		function opOfResult(node) {
			...
			if (kind === "read") {
				...
				const range = /\(Showing lines (\d+)-(\d+)(?: of \d+)?\)/.exec(text)?.[1];
		...
		*/
		function stitchSegmentReads(ops) {
			const readsByPath = /* @__PURE__ */ new Map();
			for (const op of ops) {
				if (op.kind !== "read" || op.isError || op.read === void 0) continue;
				...
				list.push({
					...op,
					readStart: rows[0].line,
					readEnd: rows[rows.length - 1].line
				});
			...
			for (const segs of readsByPath.values()) {
				...
				const flush = () => {
					...
					for (const seg of chain) for (const row of parseReadLines(seg.read ?? "")) {
				...
				for (const seg of segs) {
					...
					if (prev !== void 0 && (seg.readStart ?? 0) > (prev.readEnd ?? -Infinity) + 1) flush();
					chain.push(seg);
				}
				flush();
			}
		...
		*/
		...
		const UPDATE_ID$1 = "dsh-file-trace";
		...
		*/
		...
		async function latestFromHost() {
			try {
				const res = await fetch(`/${UPDATE_ID$1}/latest`, {
					method: "GET",
    ...
    repeat return then true until while`),
		...
		*/
		...
		var FileTrace_module_css_default = {
			...
			"mdMermaidHost": "GHGoAa_mdMermaidHost",
			"opRow": "GHGoAa_opRow",
			...
			"mdMermaidOverlay": "GHGoAa_mdMermaidOverlay",
			...
			"mdMermaidStage": "GHGoAa_mdMermaidStage",
			"sign": "GHGoAa_sign",
		...
		*  math block or inline math is rendered. */
		...
		let katexPromise;
		function loadKatexChunk() {
			katexPromise = katexPromise ?? import(KATEX_CHUNK_URL);
			return katexPromise;
		}
		...
		*  pre-0.3.14 styled span, so a missing chunk degrades, never breaks. */
		function InlineMath({ tex }) {
			...
			(0, react.useEffect)(() => {
				let alive = true;
				loadKatexChunk().then((mod) => {
					if (alive) setHtml(mod.renderMath(tex, false));
				}).catch((cause) => {
					console.warn("[dsh-file-trace] katex chunk fell back to raw text:", cause);
				});
				return () => {
					alive = false;
				};
			}, [tex]);
			if (html === null) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: FileTrace_module_css_default.mdMath,
				children: tex
			});
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: FileTrace_module_css_default.mdMath,
				"data-katex-state": "rendered",
		...
		*  the same raw-text fallback as InlineMath. */
		function MathBlock({ tex }) {
			...
			(0, react.useEffect)(() => {
				let alive = true;
				loadKatexChunk().then((mod) => {
					if (alive) setHtml(mod.renderMath(tex, true));
				}).catch((cause) => {
					console.warn("[dsh-file-trace] katex chunk fell back to raw text:", cause);
				});
				return () => {
			...
			if (html === null) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		...
		*  to close — the SVG stays the already-sanitized string from the chunk. */
		function MermaidZoom({ svg, onClose }) {
			...
			const zoom = (delta, cx, cy) => {
				...
				const next = Math.min(8, Math.max(.2, current.scale * delta));
			...
			const tool = (label, title, onClick) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
				type: "button",
				className: FileTrace_module_css_default.mdMermaidTool,
				title,
				"aria-label": title,
				onClick,
				children: label
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				id: "dsh-file-trace-mermaid-modal",
				className: FileTrace_module_css_default.mdMermaidOverlay,
				onClick: (e) => {
					if (e.target === e.currentTarget) onClose();
				},
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: FileTrace_module_css_default.mdMermaidTools,
					"data-modal-tools": true,
					children: [
						tool("＋", "放大", () => {
							zoom(1.2);
						}),
					...
					ref: stageRef,
					className: FileTrace_module_css_default.mdMermaidStage,
					onPointerDown: (e) => {
						e.preventDefault();
						dragRef.current = {
							active: true,
							startX: e.clientX - zoomRef.current.tx,
							startY: e.clientY - zoomRef.current.ty
						};
					},
					onPointerMove: (e) => {
						if (!dragRef.current.active) return;
						zoomRef.current.tx = e.clientX - dragRef.current.startX;
						zoomRef.current.ty = e.clientY - dragRef.current.startY;
						apply();
					},
					onPointerUp: () => {
					...
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						ref: hostRef,
						className: FileTrace_module_css_default.mdMermaidHost,
		...
		function MermaidBlock({ code }) {
			...
			const [zoomed, setZoomed] = (0, react.useState)(false);
			(0, react.useEffect)(() => {
				let alive = true;
				setSvg(null);
				setFailed(false);
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				title: "点击放大：滚轮缩放 / 拖拽平移",
				onClick: () => {
					setZoomed(true);
				},
				dangerouslySetInnerHTML: { __html: svg }
			}), zoomed && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(MermaidZoom, {
				svg,
				onClose: () => {
					setZoomed(false);
				}
		...
		*/
		function parseBlocks(src) {
			...
			if (lines[0] !== void 0 && lines[0].trim() === "---") {
				...
				if (j < lines.length && first !== void 0 && /^[ \t]*[A-Za-z][\w.-]*[ \t]*:(\s|$)/.test(first)) {
					blocks.push({
						kind: "frontmatter",
		...
		*/
		function renderInline(text, keyBase = "i", rctx = {}) {
			...
			while (pos < text.length) {
				...
				if (ch === "$") {
					...
					if (m !== null) {
						flush();
						nodes.push(/* @__PURE__ */ (0, react_jsx_runtime.jsx)(InlineMath, { tex: m[1] }, key()));
						pos += m[0].length;
						continue;
					}
				}
				...
				for (const e of EMPHASIS) {
					...
					const re = new RegExp(escapeRe$1(e.marker) + "(?=\\S)([\\s\\S]*?\\S)" + escapeRe$1(e.marker));
		...
		function renderBlock(b, key, rctx) {
			switch (b.kind) {
				...
				case "math": return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(MathBlock, { tex: b.text }, key);
		...
		*  wrong (old host process / proxy caches application/octet-stream). */
		function PdfFrame(props) {
			...
			if (state.kind === "loading") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				"data-file-trace-pdf-loading": true,
				children: props.t("pdf.loading")
			});
			if (state.kind === "error") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: FileTrace_module_css_default.mdPane,
				"data-file-trace-pdf-error": true,
				role: "alert",
				children: state.message
			});
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: FileTrace_module_css_default.mdPane,
				"data-file-trace-pdf-pane": true,
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("iframe", {
					className: FileTrace_module_css_default.htmlFrame,
					title: props.path,
					src: state.url
				})
			});
		...
		*  and no host route is involved. */
		function SvgFrame(props) {
			const [state, setState] = (0, react.useState)({ kind: "loading" });
			const [imgFailed, setImgFailed] = (0, react.useState)(false);
			(0, react.useEffect)(() => {
				const controller = new AbortController();
				let objectUrl;
				setState({ kind: "loading" });
				(async () => {
					...
					try {
						if (controller.signal.aborted) return;
						if (new DOMParser().parseFromString(props.src, "image/svg+xml").getElementsByTagName("parsererror").length > 0) throw new Error("payload XML invalid");
			...
			if (state.kind === "loading") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: FileTrace_module_css_default.mdPane,
				"data-file-trace-svg-pane": true,
				"data-svg-state": "loading",
				children: props.t("pdf.loading")
			});
			if (state.kind === "error") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: FileTrace_module_css_default.mdPane,
				"data-file-trace-svg-missing": true,
				role: "alert",
				children: props.t("svg.missing")
			});
			if (imgFailed) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: FileTrace_module_css_default.mdPane,
				"data-file-trace-svg-pane": true,
				"data-svg-state": "iframe",
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("iframe", {
					className: FileTrace_module_css_default.htmlFrame,
					title: props.path,
					src: state.url,
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: FileTrace_module_css_default.mdPane,
				"data-file-trace-svg-pane": true,
				"data-svg-state": state.via === "asset" ? "img-asset" : "img-payload",
		...
		* content and diff whole files; otherwise fall back to the snippet itself. */
		...
		function PresentedPane({ path, sessionId: sessionProp, t }) {
			const [state, setState] = (0, react.useState)({ kind: "loading" });
			(0, react.useEffect)(() => {
				const controller = new AbortController();
				setState({ kind: "loading" });
				const sessionId = sessionProp !== void 0 && sessionProp !== "" ? sessionProp : currentSessionIdStore.value;
				const url = /^[a-zA-Z]:[\\/]/.test(path) || path.startsWith("/") || path.startsWith("\\\\") ? assetUrl(path) : sessionId === "" ? assetUrl(path) : `${assetUrl(path)}&session=${encodeURIComponent(sessionId)}`;
			...
			if (state.kind === "loading") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: FileTrace_module_css_default.mdPane,
				"data-file-trace-present-loading": true,
				children: t("present.loading")
			});
			if (state.kind === "error") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: FileTrace_module_css_default.mdPane,
				"data-file-trace-present-error": true,
				role: "alert",
				children: t("present.missing")
			});
			if (isMarkdownPath(path)) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: FileTrace_module_css_default.readContent,
				"data-file-trace-present": true,
		...
		function FileTraceButton({ useConversation, t, sessionId }) {
			const rawOps = useConversation((conversation) => {
				const chat = conversation.views.get("chat");
				return extractFileOps(chat?.legacy.nodes ?? [], chat?.legacy.runningCalls ?? []);
			});
			const [stitchOn, setStitchOn] = (0, react.useState)(false);
			const ops = (0, react.useMemo)(() => stitchOn ? stitchSegmentReads(rawOps) : rawOps, [rawOps, stitchOn]);
			const stitchable = (0, react.useMemo)(() => {
				const readsByPath = /* @__PURE__ */ new Map();
				for (const op of rawOps) {
					if (op.kind !== "read" || op.read === void 0) continue;
			...
			const [htmlReading, setHtmlReading] = (0, react.useState)(false);
			const [htmlSandbox, setHtmlSandbox] = (0, react.useState)("script");
			const [pdfReading, setPdfReading] = (0, react.useState)(false);
			const [svgReading, setSvgReading] = (0, react.useState)(false);
			const SANDBOX_ATTR = {
				strict: "",
				script: "allow-scripts",
				relaxed: "allow-scripts"
			};
			const PERMISSIONS_ATTR = {
				strict: "",
				script: "",
			...
			* into a full-height right sidebar. */
			...
			const [redactionOn, setRedactionOn] = (0, react.useState)(() => {
				try {
					return localStorage.getItem("dsh-file-trace.redaction") !== "0";
				} catch {
					return true;
				}
			});
			const toggleRedaction = () => {
				setRedactionOn((prev) => {
					const next = !prev;
					try {
						localStorage.setItem("dsh-file-trace.redaction", next ? "1" : "0");
					} catch {}
					return next;
				});
			};
			const viewOp = (0, react.useMemo)(() => {
				const priorRaw = selectedOp === void 0 ? void 0 : knownContentBefore(ops, selected?.path ?? "", selectedOp);
				if (selectedOp === void 0 || !redactionOn) return {
					op: selectedOp,
					prior: priorRaw,
					hit: false
				};
				const path = selected?.path ?? "";
				const mask = (text) => redactText(path, text).text;
				const hit = [
					selectedOp.read,
					selectedOp.content,
					selectedOp.edit?.oldString,
					selectedOp.edit?.newString,
					selectedOp.errorText,
					priorRaw
				].some((text) => text !== void 0 && redactText(path, text).hit);
				return {
					op: {
						...selectedOp,
						...selectedOp.read !== void 0 ? { read: mask(selectedOp.read) } : {},
						...selectedOp.content !== void 0 ? { content: mask(selectedOp.content) } : {},
						...selectedOp.edit !== void 0 ? { edit: {
							oldString: mask(selectedOp.edit.oldString),
							newString: mask(selectedOp.edit.newString)
						} } : {},
						...selectedOp.errorText !== void 0 ? { errorText: mask(selectedOp.errorText) } : {}
					},
					prior: priorRaw === void 0 ? void 0 : mask(priorRaw),
					hit
				};
			}, [
				selectedOp,
			...
			const diffRows = (0, react.useMemo)(() => viewOp.op === void 0 ? [] : diffOf(viewOp.op, viewOp.prior), [viewOp]);
			...
			const readRows = (0, react.useMemo)(() => {
				if (viewOp.op?.kind !== "read" || viewOp.op.read === void 0) return [];
				let state = false;
			...
			const svgSrc = (0, react.useMemo)(() => {
				if (selected === null || !isSvgPath(selected.path)) return void 0;
				const path = selected.path;
				if (readingSrc.includes("</svg>")) return readingSrc;
				let newest;
				for (const op of ops) {
					if (op.path !== path || op.kind !== "read" || op.isError || op.read === void 0) continue;
					const text = op.readClean ? op.read : parseReadContent(op.read);
					if (!text.includes("</svg>")) continue;
					if (newest === void 0 || op.time > newest.at) newest = {
						at: op.time,
						text
					};
				}
			...
			(0, react.useEffect)(() => {
				...
				setMdReading(false);
				setHtmlReading(false);
				setHtmlSandbox("script");
				setPdfReading(false);
				setSvgReading(false);
			}, [selectedOp]);
			(0, react.useEffect)(() => {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							stitchable && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: FileTrace_module_css_default.readModeBtn,
								"data-on": stitchOn ? "true" : void 0,
								onClick: () => {
									setStitchOn((v) => !v);
								},
								title: t("stitch.do"),
								children: stitchOn ? t("stitch.on") : t("stitch.do")
							}),
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									...
									viewOp.hit && redactionOn && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: FileTrace_module_css_default.redactBanner,
										role: "status",
										children: t("redact.banner")
									}),
									viewOp.op?.partial === true && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: FileTrace_module_css_default.redactBanner,
										role: "status",
										children: t("read.partial", { range: viewOp.op.partialRange ?? "" })
									}),
									viewOp.op?.readStitched !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: FileTrace_module_css_default.redactBanner,
										role: "status",
										children: t("read.stitched", {
											range: viewOp.op.readStitched,
											count: String(viewOp.op.partialSegments ?? 0)
										})
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: FileTrace_module_css_default.readModeBtn,
										"data-on": redactionOn ? "true" : void 0,
										onClick: toggleRedaction,
										title: redactionOn ? t("redact.off") : t("redact.on"),
										children: redactionOn ? t("redact.onLabel") : t("redact.offLabel")
									}),
									...
									isHtmlPath(selected.path) && !selected.op.isError && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: FileTrace_module_css_default.readModeBtn,
										"data-on": htmlReading ? "true" : void 0,
										onClick: () => {
											setHtmlReading((prev) => !prev);
										},
										title: htmlReading ? t("html.raw") : t("html.render"),
										children: htmlReading ? t("html.raw") : t("html.render")
									}),
									htmlReading && isHtmlPath(selected.path) && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: FileTrace_module_css_default.readModeBtn,
										"data-on": "true",
										onClick: () => {
											setHtmlSandbox((prev) => prev === "strict" ? "script" : prev === "script" ? "relaxed" : "strict");
										},
										title: t(`html.sandbox.${htmlSandbox}`),
										children: t(`html.sandbox.${htmlSandbox}`)
									}),
									isPdfPath(selected.path) && isAbsolutePath(selected.path) && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: FileTrace_module_css_default.readModeBtn,
										"data-on": pdfReading ? "true" : void 0,
										onClick: () => {
											setPdfReading((prev) => !prev);
										},
										title: pdfReading ? t("pdf.raw") : t("pdf.render"),
										children: pdfReading ? t("pdf.raw") : t("pdf.render")
									}),
									isSvgPath(selected.path) && !selected.op.isError && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: FileTrace_module_css_default.readModeBtn,
										"data-on": svgReading ? "true" : void 0,
										onClick: () => {
											setSvgReading((prev) => !prev);
										},
										title: svgReading ? t("svg.raw") : t("svg.render"),
										children: svgReading ? t("svg.raw") : t("svg.render")
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: FileTrace_module_css_default.close,
							...
							svgReading && isSvgPath(selected.path) && svgSrc === void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: FileTrace_module_css_default.mdPane,
								"data-file-trace-svg-missing": true,
								role: "alert",
								children: t("svg.missing")
							}) : svgReading && isSvgPath(selected.path) && svgSrc !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(SvgFrame, {
								path: selected.path,
								src: svgSrc,
								t
							}) : pdfReading && isPdfPath(selected.path) ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(PdfFrame, {
								path: selected.path,
								t
								...
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									...
									role: "alert",
									children: viewOp.op?.errorText ?? t("error")
								})
							...
							}) : htmlReading && isHtmlPath(selected.path) && readingSrc !== "" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: FileTrace_module_css_default.mdPane,
								"data-file-trace-html-pane": true,
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("iframe", {
									className: FileTrace_module_css_default.htmlFrame,
									title: selected.path,
							...
							}) : selected.op.presentOnly === true ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(PresentedPane, {
								path: selected.path,
								sessionId,
								t
