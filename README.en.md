...
```sh
# Option 1: pinned-tag git dependency (public mirror, recommended; github:dsh-external/dsh-file-trace also works)
dsh plugin --profile web add '@dsh-external/dsh-file-trace@github:dsh-external/dsh-file-trace#v0.3.10'

# Option 2: local link (development; cloned repos ship build artifacts, rebuild with pnpm run build after edits)
git clone https://github.com/dsh-external/dsh-file-trace.git
...
> 1. Run `dsh plugin --profile web add '@dsh-external/dsh-file-trace@github:dsh-external/dsh-file-trace#v0.3.10'` (the first run may fail because pnpm 11 blocks node-pty build scripts)
...
> On errors, first check the known limitations in the README at <https://github.com/dsh-external/dsh-file-trace>.
...
- **Math formula rendering (KaTeX, lazy)**: `$$...$$` display blocks and `$...$` inline formulas in reading mode are typeset for real via a lazily-loaded katex chunk from the host `/dsh-file-trace/resources` route (single-file bundle, CSS and all fonts inlined as base64; one load covers the whole document) — subscripts/superscripts, fractions, radicals, Greek letters, \mathrm/\sqrt/\times/\quad all render; `throwOnError: false` keeps a bad formula a red-text annotation instead of breaking the page; on chunk-load failure the view falls back to the raw source text (the pre-0.3.14 behavior) — degrade, never crash.
...
| --- | --- | --- |
| `v0.3.14` (default) | `dsh-v0.1.2-alpha.1`–`alpha.5`, `rc.1`, `0.1.3-alpha.1`–`0.1.6-alpha.1` | **Math rendering**: `$$`/`$` formulas in the Markdown reading mode are typeset for real via a lazily-loaded katex chunk (single file, fonts inlined as base64), falling back to raw text on load failure; typecheck/105 tests/build green, chunk route verified 200 on the live host; takes effect after a host restart (rows v0.3.11–v0.3.13 are recorded in the Chinese README) |
...
| `v0.3.10` | `dsh-v0.1.2-alpha.1`–`alpha.5`, `rc.1`, `0.1.3-alpha.1`–`0.1.5-alpha.2` | Declares support for 0.1.5-alpha.2 (published on npm, pinned-version real-host verified; alpha.2 changes are sidebar document previews, model file delivery, minimal default tools, and the `fs-ext` install fix — zero code delta on this plugin's client surface; typecheck/build/105 tests green, boot manifest confirms loading) |
| `v0.3.9` | `dsh-v0.1.2-alpha.1`–`alpha.5`, `rc.1`, `0.1.3-alpha.1`–`0.1.5-alpha.1` | Declares support for 0.1.5-alpha.1 (published on npm, pinned-version real-host verified; 0.1.5 changes sit in session format V3 / ctx.agent removal / host bundle serving route, zero code delta on this plugin's client surface; typecheck/build/105 tests green, boot manifest confirms loading) |
| `v0.3.8` | same as v0.3.7 | **SVG render preview**: a Render/Raw toggle on `.svg` op previews with a three-level source chain (host asset route bytes → session payload blob after DOMParser validation → sandboxed iframe fallback, scripts blocked, SMIL animations intact); local `.svg` embeds in markdown reading mode render as images instead of file chips; invalid payload XML shows an explicit error instead of a silent broken image |
| `v0.3.7` | `dsh-v0.1.2-alpha.1`–`alpha.5`, `rc.1`, `0.1.3-alpha.1`, `0.1.3-alpha.2` | Fix: a document starting with a `---` divider followed by a later divider swallowed the heading and body as YAML frontmatter (client renderer fix, DSH-independent) |
