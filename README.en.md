...
dsh plugin --profile web add '@dsh-external/dsh-file-trace@github:lhh010/dsh-file-trace#v0.3.4'
...
> 1. Run `dsh plugin --profile web add '@dsh-external/dsh-file-trace@github:lhh010/dsh-file-trace#v0.3.4'` (the first run may fail because pnpm 11 blocks node-pty build scripts)
...
| --- | --- | --- |
| `v0.3.4` (default) | `dsh-v0.1.2-alpha.1`–`alpha.5`, `rc.1`, `0.1.3-alpha.1` | **PDF render preview**: `.pdf` ops (read / write / edit) gain a Render/Raw toggle — the file bytes stream through the host asset route into an explicitly-typed `application/pdf` Blob, opened by the browser's native PDF viewer; absolute paths only (a relative op path cannot locate the session workspace) |
