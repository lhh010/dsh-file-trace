...
> 1. 执行 `dsh plugin --profile web add '@dsh-external/dsh-file-trace@github:lhh010/dsh-file-trace#v0.3.1'`（首次可能被 pnpm 11 拦截 node-pty 构建脚本而失败）
...
| 插件版本 | DSH 版本 | 说明 |
| --- | --- | --- |
| `v0.3.1`（默认） | `dsh-v0.1.2-alpha.1`~`alpha.5`、`rc.1` | 文档更新：明确脱敏层定位——展示层便利（截图 / 分享场景），非安全边界，会话日志保留原始内容 |
