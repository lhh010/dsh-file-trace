...
> 兼容 DSH `dsh-v0.1.2-alpha.3`（typecheck/build + 74 单测全绿，实机验证；本插件基于 alpha.1+ 编写，rc.2 下无可用版本）
...
```sh
# 方式一：git 依赖固定 tag（公开镜像，推荐；也可用 github:dsh-external/dsh-file-trace）
dsh plugin --profile web add '@dsh-external/dsh-file-trace@github:dsh-external/dsh-file-trace#v0.2.3'
...
> 1. 执行 `dsh plugin --profile web add '@dsh-external/dsh-file-trace@github:dsh-external/dsh-file-trace#v0.2.3'`（首次可能被 pnpm 11 拦截 node-pty 构建脚本而失败）
...
> 遇到报错先查 https://github.com/dsh-external/dsh-file-trace README 的已知限制。
...
- **Mermaid 图渲染（懒加载 + 降级）**：阅读模式下 ```mermaid 围栏会按需从宿主 `/dsh-file-trace/resources` 懒加载 mermaid chunk 渲染成图（流程图/时序图/状态图/Gantt 等）；加载失败或离线时自动回退为原样代码块，不影响阅读。
...
| 插件版本 | DSH 版本 | 说明 |
| --- | --- | --- |
| `v0.2.3`（默认） | `dsh-v0.1.2-alpha.1`~`alpha.3` | Mermaid 懒加载渲染（失败回退代码块）；新增宿主 chunk 资源路由 |
