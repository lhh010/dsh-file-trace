...
> 1. 执行 `dsh plugin --profile web add '@dsh-external/dsh-file-trace@github:dsh-external/dsh-file-trace#v0.2.8'`（首次可能被 pnpm 11 拦截 node-pty 构建脚本而失败）
...
| 插件版本 | DSH 版本 | 说明 |
| --- | --- | --- |
| `v0.2.8`（默认） | `dsh-v0.1.2-alpha.1`~`alpha.5` | LaTeX/TeX 语法高亮（TeXstudio 风格初步渲染：命令→macro、数学→string、注释→灰、{}&^_#→structure、200+ 关键字） |
