...
> 1. 执行 `dsh plugin --profile web add '@dsh-external/dsh-file-trace@github:dsh-external/dsh-file-trace#v0.1.3'`（首次可能被 pnpm 11 拦截 node-pty 构建脚本而失败）
...
- **读取视图**：显示被读取文件的**真实内容与真实行号**（剥掉 DSH 读工具响应外壳）；读取出错时以红色错误块展示且不带行号。
...
| 插件版本 | DSH 版本 | 说明 |
| --- | --- | --- |
| `v0.1.3`（默认） | `dsh-v0.1.2-alpha.1` | 右缘吸附为右侧栏 + 主对话避让；typecheck、20 个单测、构建全绿 |
