...
dsh plugin --profile web add '@dsh-external/dsh-file-trace@github:dsh-external/dsh-file-trace#v0.3.12'
...
> 1. 执行 `dsh plugin --profile web add '@dsh-external/dsh-file-trace@github:dsh-external/dsh-file-trace#v0.3.12'`（首次可能被 pnpm 11 拦截 node-pty 构建脚本而失败）
...
| 插件版本 | DSH 版本 | 说明 |
| --- | --- | --- |
| `v0.3.18`（默认） | `dsh-v0.1.2-alpha.1`~`alpha.5`、`rc.1`、`0.1.3-alpha.1`~`0.1.7-rc.1` | **交付文件内容直显**：present 类交付（代码写入、无内联内容的文件）不再显示「变更前的内容不在当前窗口，显示为全新增」——改为经宿主 asset 路由读取当前盘上内容完整渲染（.md 走 Markdown 阅读模式含图片/公式/代码高亮，其它文本带行号）；asset 路由新增 md/markdown/txt/json/csv/log 文本类型。typecheck/113 单测/构建全绿 |
